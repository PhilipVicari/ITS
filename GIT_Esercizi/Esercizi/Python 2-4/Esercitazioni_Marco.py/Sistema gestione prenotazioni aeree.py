...
# e la capacità massima di posti disponibili sul volo (numero intero) che sono attributi passati come argomenti in input. Inoltre, 
# la classe deve avere un attributo chiamato prenotazioni, il quale non deve essere passato come argomento del costruttore; 
...
# Il costruttore non deve ricevere in input questi argomenti, 
# ma assegnare loro un valore iniziale tale che la somma di questi tre valori interi sia uguale al numero dei posti disponibili sul volo. 
...
# Inoltre, nel caso in cui sia possibile prenotare un posto, 
...
# (specificando il codice del volo). 
...
# poi aggiornare gli attributi prenotazioni_economica, prenotazioni_business, prenotazioni_prima 
# con l'esatto numero delle prenotaioni ricevute per ogni classe di servizio. 
# 
# Suggerimento: introdurre delle variabili locali d'appoggio per gestire le prenotazioni per ogni classe di servizio da azzerare 
# all'inizio di ogni fase di prenotazione.
...
# associare come rispettivo valore il numero di posti che sono rimasti disponibili nella classe economica, 
...
# Nel caso in cui la prenotazione charter andasse a buon fine, il metodo deve stampare a schermo un messaggio in cui avvisa il tour operator che il volo charter 
# (specificandone il codice del volo) è stato prenotato completamente, mostrando anche il prezzo pagato. 
...
# Questa classe deve avere un costruttore che richiede come argomento in input solo il nome della compagnia (stringa) ed 
# il prezzo standard di un biglietto (numero float), e creare una lista vuota chiamata flotta. 
...
# il prezzo  per un posto in classe economica è pari a prezzo standard, il prezzo per un posto in classe business è pari al doppio del prezzo standard, 
...
#     provare ad effettuare un secondo tentativo di prenotazione. In questo caso, la prenotazione dovrebbe fallire, 
#     in quanto il volo charter dovrebbe essere già prenotato.
