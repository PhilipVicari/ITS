...
file_path2 = "utente.json"
Utenti = JsonDeserialize(file_path2)

@api.route('/getcredenziali', methods=['POST'])
def GestisciGetCredenziali():
    content_type = request.headers.get('Content-Type')
    if content_type == 'application/json':
        jsonReq = request.json
        Username = jsonReq.get('username')
        Password = jsonReq.get('Password')
        if Username in Utenti and Password in Utenti[Username]["Password"]:
            return jsonify({"Esito": "200", "Msg": "Utente Trovato"}), 200
        else:
            Utenti[Username] = jsonReq
            JsonSerialize(Utenti, file_path2) 
            return jsonify({"Esito": "404", "Msg": "Utente Inesistente"}), 200
    else:
